            ...
            <div className='px-6 pt-4 pb-6'>
                <div className='flex gap-2 mb-4'>
                    {project.badges.map((badge, key) => (
                        <span key={key} className='bg-blue-900 text-indigo-200 text-xs px-2 py-1 rounded-full'>{badge}</span>
                    ))}
                </div>
                <Link href={project.linkRepo} passHref target='_blank'>
                ...
                <Link href={project.linkDemo!} passHref target='_blank'>
                    <button id={`${styles.demoBtn}`} className='w-full bg-indigo-600 text-white font-bold mt-2 py-2 px-4 rounded-lg transition-all duration-300 transform hover:translate-y-[-3px] hover:shadow-lg' disabled={project.linkDemo === '' || project.linkDemo === undefined}>
                        Demo
                    </button>
                </Link>
            </div>
